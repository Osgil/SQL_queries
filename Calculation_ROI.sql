...
with ads_users as (SELECT ads_campaign,
                   ...
                   GROUP BY ads_campaign, user_id, order_id having count(order_id) >= 1)
SELECT ads_campaign,
       round((sum_revenue_by_groups - 250000) / 250000 * 100, 2) as roi
FROM   (SELECT ads_campaign,
               sum(price) as sum_revenue_by_groups
        FROM   (SELECT *
                FROM   ads_users) active_users join (SELECT order_id,
                                                    unnest(product_ids) as product_id
                                             FROM   orders) orders_content using(order_id) join (SELECT product_id,
                                                                   price
                                                            FROM   products) product_price using(product_id)
        GROUP BY ads_campaign) f_table