...
-- *Длинное перечисление user_id, которые представлены в запросе ниже*
...
-- *Примечание: Стоимость привлечения активных пользователей, то есть сделавших 1 или более заказов, которые не были отменены

SELECT ads_campaign,
       round(250000 / count(user_id)::decimal, 2) as cac
FROM   (SELECT ads_campaign,
        ...
        GROUP BY ads_campaign, user_id having count(order_id) >= 1) t1
